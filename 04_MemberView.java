...
// View : 사용자가 보게 될 시각적인 요소(화면)를 담당하는 곳 (출력과 입력) 
//        항상 출력문과 입력문들은 View 단에서만 작성할 것!
public class MemberView {

	//전역으로 다 쓸 수 있게끔 Scanner 객체 생성 (전역변수)
	private Scanner sc = new Scanner(System.in);	
	//전역으로 다 쓸 수 있게끔 
	//어느 메소드에서든 간에 바로 MemberController 의 메소드로 요청할 수 있게끔 객체 생성(전역변수) 
	...
	//1.회원 추가용 화면 : 추가하고자 하는 회원의 정보를 키보드로 입력받아서 추가요청할 수 있는 화면
	public void insertMember() {
	    ...
	    // 필요한 데이터를 다 입력받았다면
	    // 회원을 추가하고자 요청 => MemberController 의 어떤 메소드를 호출
	    mc.insertMember(userId,userPwd,userName,gender,age,email,phone,address,hobby);
    
	}	
	//2.회원 전체 조회를 할 수 있는 화면
	public void selectAll() {
		
		System.out.println("----회원 전체 조회 ----");
		
		// 이 경우에는 따로 입력받을만한 값이 없음
		// 곧바로 MemberController 의 메소드를 호출하여 바로 요청을 넘길것임!
		mc.selectAll(); // 입력받은 값이 없기 때문에 넘길 값 또한 없음! 	
	}
	//3.회원 아이디로 검색할 수 있는 화면
	public void selectByUserId() {
		...
		// MemberController 의 어떤 메소드를 호출해서 아이디 검색 요청
	...
	public void selectByUserName() {
		...
		//MemberController 의 어떤 메소드를 호출해서 이름 검색 요청
	...
	public void updateMember() {
		...
		// 바꿀 내용물들 - 비밀번호, 이메일, 전화번호, 주소 (SET절에 들어갈 내용)
		...
		//MemberController 의 어떤 메소드에게 요청 처리 (위의 매개변수를 넘길 예정)
	...
	public void deleteMember() {
	 ...
	 //MemberController 의 어떤 메소드를 호출해서 탈퇴 요청 처리
	...
	// 조회 서비스 요청 시 여러 행이 조회된 결과를 받아서 보게 될 화면을 만드는 메소드
	...
	// 조회 서비스 요청 시 한개의 행이 조회된 결과를 받아서 보게 될 화면을 만든 메소드
